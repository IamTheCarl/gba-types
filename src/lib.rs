...
bitstruct_newtype! {
  ...
  /// register.
  ///
  /// * When used with `IE`, the set bits determine the interrupts that will be
  /// accepted. Each interrupt source must be separately configured
  ///   to also send the appropriate interrupt via other registers.
  /// * When used with `IF`:
  ///   * Read the register to see what interrupts are currently pending.
  ///   * Write the register with active bits to clear a pending interrupt.
  ///     It's slightly confusing that you write a 1-bit to change a 1-bit to a
  ///     0-bit, but that's really how it works.
...
bitstruct_newtype! {
  ...
  /// as wait state 1, and 0x0C000000 as wait state 2. All of these addresses
  /// mirror content of the game pak ROM. The address that Rust will access the
  /// ROM through is dependant on your linker script. If you use ours (provided
  /// by the gba crate) then the default is 0x08000000.
  WaitControlSetting(u16) {
    ...
    /// PHI Terminal Output speed. Usage documentation is unclear. Is likely
    /// for strange Nintendo peripherals.
    [11-12 => PhiTerminalOutput: phi_terminal, set_phi_terminal],
    /// Game Pak Prefetch Buffer enable. When enabled, the GBA will attempt to
    /// fetch the next CPU instruction before it is needed, thus brining the
    /// wait down to zero cycles. This, howerver, is dependent on the bus being
    /// free at some point long enough to do this.
